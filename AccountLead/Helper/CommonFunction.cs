    ...
    public class CommonFunction : ICommonFunction
    {
        ...
        public async Task<JArray> getAccountLeadData(string AccountID)
        {
            try
            {
                ...
                string query_url = $"eqs_ddeaccounts()?$select=eqs_ddeaccountid,_eqs_leadaccountid_value,eqs_accountownershipcode,_eqs_productid_value,_eqs_productcategoryid_value&$filter=_eqs_leadaccountid_value eq '{leadaccount_id}' and eqs_dataentrystage eq {Stage}";
